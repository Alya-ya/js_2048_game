...
class Game {
   ...
   */ constructor(initialState){
        if (initialState) // копія масиву
        this._board = initialState.map((row)=>row.slice());
        else // порожнє поле 4×4
    ...
    moveLeft() {
        ...
        for(let i = 0; i < 4; i++){
            ...
            for(let s = 0; s < row.length - 1; s++)if (row[s] === row[s + 1]) {
                row[s] *= 2;
                this._score += row[s];
                row[s + 1] = 0;
            }
   ...
   * Resets the game.
   */ restart() {
        for(let k = 0; k < 4; k++)for(let l = 0; l < 4; l++)this._board[k][l] = 0;
        this._score = 0;
        this._status = 'idle';
        this.addRandomTile();
        this.addRandomTile();
    ...
    addRandomTile() {
        const emptyCells = [];
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (this._board[r][c] === 0) emptyCells.push([
                r,
                c
            ]);
        }
    ...
    /* eslint-disable comma-dangle */ transpose() {
        this._board = this._board[0].map((_, colIndex)=>this._board.map((row)=>row[colIndex]));
    }
    /* eslint-enable comma-dangle */ checkWin() {
        let found = false;
        for(let g = 0; g < this._board.length; g++){
            for(let h = 0; h < this._board.length; h++)if (this._board[g][h] === 2048) {
                this._status = 'win';
                found = true;
            }
        }
        if (found === false) this._status = 'playing';
    }
    checkLose() {
        let canMove = false;
        for(let g = 0; g < this._board.length; g++)for(let h = 0; h < this._board.length; h++){
            if (this._board[g][h] === 0) canMove = true;
            if (h < 3 && this._board[g][h] === this._board[g][h + 1]) canMove = true;
            if (g < 3 && this._board[g][h] === this._board[g + 1][h]) canMove = true;
        }
        if (canMove === false) this._status = 'lose';
    }
}
module.exports = Game;
